...
export async function execute(interaction) {
    ...
    if (result == 1) {
        await interaction.reply({content: "Model " + model + " was updated with the message: " + message, flags: [MessageFlags.Ephemeral]});
    } else {
        await interaction.reply({content: "Model " + model + " was not updated", flags: [MessageFlags.Ephemeral]});
    }
