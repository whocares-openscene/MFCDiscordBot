  ...
  for (let index = 0; index < models.length; index++) {
    ...
    if (Object.hasOwn(allmodels, name)) {
      ...
      if (modelstatus["show_kind"] === 2 && element["topic"] != 2) {
        ...
        (Number.isInteger(element["int"]) || element["int"] === false) &&
        modelstatus["show_kind"] === 1 &&
        typeof modelstatus["subject"] != "undefined"
      ) {
        // Public show
        const image = await webstuff.getPicture(
          modelstatus["image_url"],
          element["modelid"],
        );
        const message =
          element["message"] + "\nCurrent topic is:\n" + modelstatus["subject"];
      ...
      const message =
        element["modelname"] +
        " has gone offline. She was online for " +
        time.getUTCHours() +
        " hours " +
        time.getUTCMinutes() +
        " minutes and " +
      ...
      const message =
        element["modelname"] + " has gone offline while the bot was offline.";
      await channel.send(message);
      await dbfunctions.updatetime(
        element["modelid"],
        false,
