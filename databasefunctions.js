...
export async function getmodel(modelid, channel) {
  return new Promise((resolve, reject) => {
    db.get(
      "SELECT * FROM models WHERE modelid = ? AND channel = ?",
      [modelid, channel],
      (err, row) => {
        if (err) return reject(err);
        resolve(row);
...
export async function gettopic(modelid, channel) {
  return new Promise((resolve, reject) => {
    db.get(
      "SELECT topic FROM models WHERE modelid = ? AND channel = ?",
      [modelid, channel],
      (err, row) => {
        if (err) return reject(err);
...
export async function gettime(modelid, channel) {
  return new Promise((resolve, reject) => {
    db.get(
      "SELECT time FROM models WHERE modelid = ? AND channel = ?",
      [modelid, channel],
      (err, row) => {
        if (err) return reject(err);
...
export async function updatetopic(modelid, topic, channel) {
  return new Promise((resolve, reject) => {
    db.run(
      "UPDATE models SET topic = ? WHERE modelid = ? AND channel = ?",
      [topic, modelid, channel],
      function (err) {
        if (err) return reject(err);
        resolve(this.changes);
      },
...
export async function updatetime(modelid, time, channel) {
  return new Promise((resolve, reject) => {
    db.run(
      "UPDATE models SET time = ? WHERE modelid = ? AND channel = ?",
      [time, modelid, channel],
      function (err) {
        if (err) return reject(err);
        resolve(this.changes);
      },
